...
TotalCommanderDirectoryChanger::TotalCommanderDirectoryChanger(wstring exe_path) : TerminalDirectoryChanger(exe_path, 0) {}
...
{
    wchar_t* cstr = _wgetenv(L"COMMANDER_EXE");

    if (cstr)
    {
        wstring COMMANDER_EXE(cstr);
        PROCESS_INFORMATION processInformation;
        STARTUPINFOW info = {sizeof(info)};
    ...
    else
    {
        ...
        wstring commandLine = to_wstring(pid) + L" -d " + to_wstring(get_delay()) + L" -kd " + to_wstring(VK_RIGHT) + L" -ku " + to_wstring(VK_RIGHT) + L" -t " + prepared_command + L" -kd " + to_wstring(VK_RETURN) + L" -ku " + to_wstring(VK_RETURN);
        wstring terminal_sender = (EXE_DIR/L"terminal-sender"/L"terminal-sender.exe").wstring();
        commandLine = terminal_sender + L" " + commandLine;
        CreateProcessW(terminal_sender.c_str(), const_cast<wchar_t*>(commandLine.c_str()), NULL, NULL, false, CREATE_NO_WINDOW, NULL, NULL, &info, &processInformation);
        CloseHandle(processInformation.hProcess);
